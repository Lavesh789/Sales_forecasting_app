...
day_of_week = int(st.sidebar.selectbox(
    "Day of Week", 
...
# Main Forecast Generation Trigger
...
if st.button("Generate Future Forecast", type="primary"):
    if model is not None:
        ...
        promotion_flag = int(1 if discount_percentage > 0 else 0)
        local_event_flag = 0
        holiday_flag = 0
        has_holiday_name = 0
        economic_indicator = 1.0

        promo_and_weekend = int(promotion_flag * is_weekend)
        promo_and_holiday = int(promotion_flag * holiday_flag)
        instock_and_promo = int(stock_avail * promotion_flag)

        # Construct Base Feature Map
        raw_feature_dict = {
            'Price': price,
        ...
        # Retrieve exact feature order from pipeline model if stored
        expected_cols = None
        ...
        if expected_cols:
            # Reindex strictly against model requirements without introducing NaN values
